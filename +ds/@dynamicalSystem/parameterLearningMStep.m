function obj = parameterLearningMStep(obj, updateOnly, opts)
...
if isempty(updateOnly)
    if opts.verbose; prevLLH = obj.filter('Kalman',true,[],fOpts); prevLLH  = prevLLH.infer.llh; end
    
    % ============= A and Q updates =======================================
    A           = (s.C - Bum) / s.PHI;
    if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts); fprintf('M-Step: ''A'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    ...
    if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts);fprintf('M-Step: ''Q'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    
    % ============= H and R updates =======================================
    H           = (s.B - Cum) / s.SIGMA;
    if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts); fprintf('M-Step: ''H'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    ...
    if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts); fprintf('M-Step: ''R'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    ...
    if obj.hasControl(1)
        ...
        obj.par.B = B;
        if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts); fprintf('M-Step: ''B'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    end
    if obj.hasControl(2)
        C       = (obj.y - obj.par.H * s.infer.mu(:,2:end)) * u(:,2:end)';
        C       = C \ obj.par.uu;
        obj.par.C = C;
        if opts.verbose; curLLH = obj.filter('Kalman',true,[],fOpts); fprintf('M-Step: ''C'' --- Change in LLH: %5.8f\n', curLLH.infer.llh - prevLLH); prevLLH = curLLH.infer.llh; end
    end
